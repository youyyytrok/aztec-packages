...
>>>>>>> 0ddf39290e40acc1b5429c61417f949799884519
                      ...
                      size_t edge_idx)
    {
        for (auto [extended_edge, multivariate] : zip_view(extended_edges.get_all(), multivariates.get_all())) {
            ...
            extended_edge = edge.template extend_to<BATCHED_RELATION_PARTIAL_LENGTH>();
        }
    }
    /**
     * @brief A variant of extend edges needed for zk-sumcheck.
     *
     ...
     * quadratic term extended to the same length ZK_BATCHED_LENGTH.
     * @todo extend_zk_edges and extend_edges share functionality in case of public polynomials. could we do better?
     *
     * @tparam ProverPolynomialsOrPartiallyEvaluatedMultivariates
     * @param extended_edges
     * @param multivariates
     * @param edge_idx
     * @param eval_masking_table
     */
    template <typename ProverPolynomialsOrPartiallyEvaluatedMultivariates>
    ...
    // template <typename HasZK> void func(T t) { return T(); }

    // template <true> void proveInternal() {  }

    // template <false> void prove() {  }
    ...
    SumcheckRoundUnivariate compute_univariate(
        ...
        std::optional<ZKSumcheckSetupData> zk_sumcheck_data = std::nullopt) // only submitted when Flavor HasZK
    {
        BB_OP_COUNT_TIME();
        ...
        if constexpr (Flavor::HasZK) {
            ...
            // mask the evaluations of sumcheck round univariate
...
\sum_{j=i+1}^{d-1}\left(g_{j,0}+g_{j,1}\right)\right) =  \texttt{libra_table}_{i,k} + \texttt{libra_running_sum}.
     ...
     */
    static SumcheckRoundUnivariate compute_libra_round_univariate(ZKSumcheckSetupData zk_sumcheck_data,
                                                                  size_t round_idx)
    {

        bb::Univariate<FF, BATCHED_RELATION_PARTIAL_LENGTH> libra_round_univariate;
