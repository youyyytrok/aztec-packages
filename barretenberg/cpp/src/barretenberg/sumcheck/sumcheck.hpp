...
template <typename Flavor> class SumcheckProver {
  ...
  public:
    ...
    */
    ...
    evaluations of witness polynomials, these are contained in \f$ \texttt{eval_masking_array} \f$. In order to optimize
    the computation of Sumcheck Round Univariates, we populate a table of univariates \f$
    \texttt{masking_terms_evaluations} \f$ which at the beginning contains the evaluations of polynomials \f$ \rho_j
    \cdot (1-X)\cdot X \f$ at \f$ X = 0,\ldots, D\f$. The prover commits to each \f$ \rho_j \f$ as a multilinear
    polynomial in \f$ d \f$ variables.
